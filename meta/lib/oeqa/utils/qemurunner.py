...
class QemuRunner:
    ...
    def launch(self, launch_cmd, get_ip = True, qemuparams = None, extra_bootparams = None, env = None):
        ...
        self.logger.info("waiting at most %s seconds for qemu pid (%s)" %
        ...
        self.logger.info("qemu started in %s seconds - qemu procces pid is %s (%s)" %
        ...
        self.logger.info("Output from runqemu:\n%s", out)
        self.logger.info("Waiting at most %d seconds for login banner (%s)" %
                ...
                if sock is self.server_socket:
                    ...
                    if data:
                        ...
                        if b' login:' in bootlog:
                            ...
                            stopread = True
                            reachedlogin = True
                            self.logger.info("Reached login banner in %s seconds (%s)" %
                                              (time.time() - (endtime - self.boottime),
                                              time.strftime("%D %H:%M:%S")))
                    else:
                        # no need to check if reachedlogin unless we support multiple connections
                        self.logger.info("QEMU socket disconnected before login banner reached. (%s)" %
                                          time.strftime("%D %H:%M:%S"))
        ...
        if not reachedlogin:
            if time.time() >= endtime:
                self.logger.info("Target didn't reach login banner in %d seconds (%s)" %
                                  (self.boottime, time.strftime("%D %H:%M:%S")))
